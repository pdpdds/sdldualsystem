...
static char loaderror[256];
    ...
    #define SDL20_DEBUG_LIBNAME "SDL2D.dll"
    ...
    char LoadSDL20Library()
    {
        if ((Loaded_SDL20 = LoadLibraryA(SDL20_LIBNAME)) != NULL)
            return 1;

        if ((Loaded_SDL20 = LoadLibraryA(SDL20_DEBUG_LIBNAME)) != NULL)
            return 1;
