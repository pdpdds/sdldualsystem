...
void menu_execute()
{
	switch (global.focus)
	{
		case FOCUS_START:
		{
			...
			printf("switched to STATE_WAVESTART\n");
			break;	
		}
		case FOCUS_OPTIONS:
		{
...
void bonus_aim_target(int num)
{
	...
	if (target_pts == BONUS_AIM_ERRVALUE || target_num == BONUS_AIM_ERRVALUE)
	{
		...
		printf("error: no aim target found! \n");
	}
	else
	{
	...
	if (global.enemies_alive < MAX_ENEMIES)
	{
		...
		for (i = 0; i < MAX_ENEMIES; i++)
		{
			if (!enemy_ship[i].alive)
			{
			    ...
			    {
				enemy_ship[i].crd.x1 = -(int)waves[global.wave].enemy_width + 5;
				...
				enemy_ship[i].crd.y1 = -(int)(waves[global.wave].enemy_height) + 5;
		...
		global.enemies_alive++;
		printf("enemy created. alive: %u, speed %u, mental %d \n",
...
void wave_live()
{
	...
	if ((cur_time - phase_starttime) >= phase_duration)
	{
		...
		if (global.phase < WAVE_PHASES - 1)
		{
			phase_start = 1;
			global.phase++;
			printf("phase: %u/%d. duration: %d sec, enemy freq: %d ms\n",
	...
	if ((cur_time - prev_time) >= 1000)
	{
		for (i = 0; i < BONUS_NUM; i++)
		{
			...
			if (!bonus[i].active  && rnd < bonus[i].chance[global.wave])
			{
				...
				/*printf("bonus created! #%d ,x1 %d, y1 %d\n",
			...
			{	
				if (bonus[i].scr_lifetime > 0)
				{
					bonus[i].scr_lifetime--;
				 	/* printf("#%d : dec scr_lifetime %d \n",
				  	i, bonus[i].scr_lifetime);*/
				}
				else
				{
					bonus[i].active = 0;
					printf("bonus_disappear #%d \n", i);
				}
			}
			    ...
			    int left2, int top2, int right2, int bottom2 )
{
	...
	//printf("l1 %d, t1 %d, r1 %d, b1 %d , l2 %d, t2 %d, r2 %d, b2 %d \n",
...
void bonus_check()
{
	...
	for (i = 0; i < BONUS_NUM; i++)
	{
		...
		{	
			...
			{
				printf("bonus_check() : bonus was taken %d \n", i);
			 	bonus[i].active = 0;
			 	bonus[i].on_player = 1;
...
//    printf("dp create()ship %d, part_n %d ,num %d\n",ship_number, part_number, i);
...
#if 0    
	printf("dp created %d, x1 %d, y1 %d, x2 %d, y2 %d \n",
		...
		if (dead_parts[i].alive)
		{
		    	...
		    	if (player_ship->alive)
			{
				...
				{	
					...
					{
						player_ship->alive = 0;
						printf("player killed by dead_part \n");
					}
				}
			}
...
/*
		...
		printf ("error: frame_render_time %u \n", frame_render_time);
*/
}
	...
	switch (global.state)
	{
		...
		case STATE_WAVECLEARED:
		{
			...
			if ((cur_time - global.wavecleared_time) > TIME_SHOW_WAVE_LABEL)
			{
				...
				printf("switched to the STATE_SCORE \n");
			}			
			break;
		}
		...
		case STATE_SCORE:
		{
			...
			if ((cur_time - global.wavescore_time) > TIME_SHOW_SCORE)
			{
				...
				global.state = STATE_WAVESTART;
				global.wavestart_time = SDL_GetTicks();
				printf("switched to the STATE_WAVESTART \n");
			}		
		}
