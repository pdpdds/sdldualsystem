...
EndSpear (void)
{
    ...
    EndScreen (ENDSCREEN11PIC);
    EndScreen (ENDSCREEN3PIC);
    EndScreen (ENDSCREEN4PIC);
    EndScreen (ENDSCREEN12PIC);
    EndText();
...
#define TIMEY   8
    ...
    tr /= LRpack;
#else
    if (gamestate.ratiolevels != 0) {
        kr /= gamestate.ratiolevels;
        sr /= gamestate.ratiolevels;
        tr /= gamestate.ratiolevels;
    }
#endif
...
{
    VW_FadeOut ();
    VWB_Bar (0, 0, 320, 200, 0x0);     // background

    CA_CacheGrChunk (PG13PIC);
    VWB_DrawPic (116, 68, PG13PIC);
    VW_UpdateScreen ();
...
#define PERCENT100AMT   10000
    ...
    VWB_Bar (0, 0, screenWidth/scaleFactor, screenHeight / scaleFactor - STATUSLINES + 1, VIEWCOLOR);

    if (bordercol != VIEWCOLOR)
        DrawStatusBorder (VIEWCOLOR);

    StartCPMusic (XTIPTOE_MUS);
...
// 
    ...
    if (levelinfo.backtolevel == 0)
    {
        gamestate.ratiolevels++;
        // Calculate par time
        float partime = 1.0;
        char timestr[6] = "01:00";

        if (levelinfo.parMin > 0 || levelinfo.parSec > 0) {
            partime = (float)levelinfo.parMin + ((float)levelinfo.parSec / (float)60.0);
            timestr[0] = '0' + (levelinfo.parMin / 10);
            timestr[1] = '0' + (levelinfo.parMin % 10);
            timestr[3] = '0' + (levelinfo.parSec / 10);
            timestr[4] = '0' + (levelinfo.parSec % 10);
        }
        ...
        */
        if (gamestate.TimeCount < partime * 4200)
            timeleft = (int32_t) ((partime * 4200) / 70 - sec);
    ...
    else
    {
        ...
        switch (mapon)
        {
            ...
            case 21:
                ...
                break;
            default:
                Write (13, 4, " extra level\n" " completed!");
                
                switch (levelinfo.parMin) {
                    case 0:
                        break;
                    case 2:
                        Write (10, 16, "15000 bonus!");
                        ...
                        break;
                    case 3:
                        Write (10, 16, "20000 bonus!");
                        GivePoints (20000);
                        break;
                    case 4:
                        Write (10, 16, "25000 bonus!");
                        GivePoints (25000);
                        break;
                    case 5:
                        Write (10, 16, "30000 bonus!");
                        GivePoints (30000);
                        break;
                    case 1:
                    default:
                        Write (10, 16, "10000 bonus!");
                        GivePoints (10000);
                        break;
                }
                break;
...
PreloadGraphics (void)
{
 ...
 //   DrawLevel ();
    ...
    mapname[pos] = 0;
    PrintX = 0;
    PrintY = 2; // Location to write on Screen
    fontnumber = 0;
    SETFONTCOLOR(0x24,0x0);
    US_CPrint(mapn);
    CA_CacheGrChunk (STARTFONT + 1); 
    fontnumber = 1;
    SETFONTCOLOR(0x36,0x0);
    US_CPrint(mapname); 
...
CheckHighScore (int32_t score, word other)
{
    ...
    if (n != -1)
    {
        ...
        PrintX = 16;
        fontnumber = 1;
        VWB_Bar (PrintX - 2, PrintY - 2, 145, 15, 0x1E);
        VW_UpdateScreen ();
        backcolor = 0x1E;
        fontcolor = 15;
