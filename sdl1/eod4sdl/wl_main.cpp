...
int      viewwidth;
int      viewheight;
int      halfviewheight; // optimization
int      halfviewwidth;
...
noconfig:
        ...
        mouseadjustment=5;
        #ifdef NEWMENU
        switches.textures = switches.shading = switches.weather = switches.messages = 1;
        switches.strafe = switches.alwaysrun = switches.clouds = 0;
        #endif
    }
...
void NewGame (int difficulty,int episode)
{
    ...
    // weapons are set in SetupGameLevel

    gamestate.health = 100;

    gamestate.primaryammo = 0;
    gamestate.secondaryammo = 0;
    gamestate.rocketammo = 0;
    gamestate.rifleammo = 0;
    gamestate.medkits = 0;
...
/*    if (_dos_getdiskfree(0,&dfree))
 ...
 //   //DiskFlopAnin(x,y);
    if (gamestate.difficulty == gd_extreme)
        gamestate.lives--; // save game count
 ...
 //   //DiskFlopAnin(x,y);
...
#endif
    fwrite(statat,sizeof(statat),1,file);
    checksum = DoChecksum((byte *)statat,sizeof(statat),checksum);
    ...
    fwrite(&fwall,sizeof(fwall),1,file);
    checksum = DoChecksum((byte *)&fwall,sizeof(fwall),checksum);

    //DiskFlopAnin(x,y);
    fwrite (&pwall,sizeof(pwall),1,file);
    checksum = DoChecksum((byte *)&pwall,sizeof(pwall),checksum);

    fwrite (&levelinfo,sizeof(levelinfo),1,file);
    checksum = DoChecksum((byte *)&levelinfo,sizeof(levelinfo),checksum);
...
boolean LoadTheGame(FILE *file,int x,int y)
{
    ...
    fread (statat,sizeof(statat),1,file);
    checksum = DoChecksum((byte *)statat,sizeof(statat),checksum);
    //DiskFlopAnin(x,y);
    ...
    fread (&fwall,sizeof(fwall),1,file);
    checksum = DoChecksum((byte *)&fwall,sizeof(fwall),checksum);

    fread (&pwall,sizeof(pwall),1,file);
    checksum = DoChecksum((byte *)&pwall,sizeof(pwall),checksum); 

    fread (&levelinfo,sizeof(levelinfo),1,file);
    checksum = DoChecksum((byte *)&levelinfo,sizeof(levelinfo),checksum);
    ...
    if (oldchecksum != checksum)
    {
        ...
        gamestate.lives = 1;
        gamestate.weapons = 0x3;
        gamestate.weapon =
            gamestate.chosenweapon = wp_pistol;
        gamestate.primaryammo = 8;
        gamestate.secondaryammo = 0;
        gamestate.rifleammo = 0;
        gamestate.rocketammo = 0;*/
    }
...
//
...
#ifndef SPEARDEMO
        BOMBTRIGGERSND,             1,  -1,
        LONGEXPLOSIONSND,            14, -1,
        SPIONSND,               19, -1,
        NEINSOVASSND,           20, -1,
        COLUMNBREAKSND,           21, -1,
        GRAUSIGHTSND,          29, -1,         // Trans Sight
        GRAUDEATHSND,          30, -1,         // Trans Death
        ROBOTSIGHTSND,        31, -1,         // Wilhelm Sight
        ROBOTDEATHSND,        32, -1,         // Wilhelm Death
        CLONEBOSSDEATHSND,           33, -1,         // Uber Death
        BARONSIGHTSND,         34, -1,         // Death Knight Sight
        BARONDEATHSND,         35, -1,         // Death Knight Death
        ...
        OTTOSIGHTSND,          53, -1,
        OTTODEATHSND,          54, -1,
        HEINRICHSIGHTSND,          55, -1,
        HEINRICHDEATHSND,          56, -1,
        TURRETFIRESND,          57, -1,
        DEATHSCREAM10SND,          58, -1,
        YOURANGSND,          59, -1,
        SILENCEDSND,          60, -1,
        CLOSEWOODDRSND,          61, -1,
        OPENWOODDRSND,          62, -1,
        CLOSENORMALDRSND,          63, -1,
        OPENNORMALDRSND,          64, -1,
