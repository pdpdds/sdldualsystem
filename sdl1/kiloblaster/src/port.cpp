...
void plot_vga (int16_t x, int16_t y, byte color)
{
	if (SDL_MUSTLOCK(::screen)) {
		if (SDL_LockSurface(::screen) < 0) return;
	}
...
void uncrunch (char *sourceptr, char *destptr, int16_t length)
{
	// TODO
...
{
	// TODO - getkey()?
	printf("getch() not yet implemented!\n");
	return 0;
}
...
//20190320
	std::string imfdata = (char*)((std::string*)pss->str().get()->c_str());
