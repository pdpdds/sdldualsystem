...
void saveboard(char *fname) {		// saves a board with .BRD extension
	...
	boardfile = fopen(dest, "wb");
	if (boardfile == 0) {
		rename(dest, dest2);
		boardfile = fopen(dest, "wb");
		if (!fwrite(&board, 1, sizeof(board), boardfile)) rexit(5);
	}
	else {
		if (!fwrite(&board, 1, sizeof(board), boardfile)) rexit(5);
	};
...
void loadcfg(void) {
	...
	if ((cfgfile == 0) || (filelength(cfgfile) <= 0)) {
		...
		if (fread(&cf, 1, sizeof(cf), cfgfile) == 0) 
			cf.firstthru = 1;
	};
	fclose(cfgfile);
};
...
void stat_win(void) {
	...
	wprint(&cmdvp, 45 - (strlen(tempstr) * 6), 16, 2,
		_ultoa(pl.score, tempstr, 10));					// score
	...
	for (x = 0; x <= inv_cnt(inv_triple); x++) {				// triple
		if (x <= 9) {
			...
			wprint(&cmdvp, 30, 88, 1, _itoa(x, tempstr, 10));
		}
		else wprint(&cmdvp, 22, 88, 1, _itoa(x, tempstr, 10));
	};
...
void rest(int16_t num) {
	...
	case 2:
		...
		break;
	case 3:
		do { checkctrl0(0); } while ((key != escape) && (key != 200) && (key != 208)); break;
	case 4:
		do {
			checkctrl0(0);
			upd_objs();
...
int16_t loadgame(void) {
	...
	if ((num >= 0) && (strlen(savename[num]) != 0)) {
		...
		gamefile = fopen(boardname, "rb");
		if ((gamefile == 0) || (filelength(gamefile) <= 0)) {
			fclose(gamefile);
		}
		else {
			fread(&o_pl, 1, sizeof(o_pl), gamefile);
			fread(&o_extra, 1, sizeof(o_extra), gamefile);
		};
		fclose(gamefile);
		return (1);
	};
...
void savegame(void) {
	...
	if (num >= 0) {
		...
		if ((key != escape) && (strlen(s) != 0)) {
			...
			strcat(boardname, ".");
			strcat(boardname, tempstr);
			gamefile = fopen(boardname, "wb");
			if (gamefile != 0) {
				fwrite(&o_pl, 1, sizeof(o_pl), gamefile);
				fwrite(&o_extra, 1, sizeof(o_extra), gamefile);
			};
			fclose(gamefile);
			savecfg();
		};
...
int main(int argc, char *argv[]) {
	...
	init_inv();
	cmd_win();
	do { checkctrl0(0); } while ((dx1 != 0) || (dy1 != 0) || (key != 0) || (fire1) || (fire2));
	menu();
