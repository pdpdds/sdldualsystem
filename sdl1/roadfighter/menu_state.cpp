...
int CRoadFighter::menu_cycle(void)
{
	...
	if (menu_state==1 || menu_state==2 || menu_state==3) {
		...
		if (menu_current_menu==2) {
			...
			delete menu_options_text;
			sprintf(tmp,"RIGHT : %s\nLEFT : %s\nFIRE : %s\nBACK\n",
					(menu_redefining_key==0 ? "":(SDL_GetKeyName((SDLKey)right_key))),
					(menu_redefining_key==1 ? "":(SDL_GetKeyName((SDLKey)left_key))),
					(menu_redefining_key==2 ? "":(SDL_GetKeyName((SDLKey)fire_key))));
			menu_options_text=new char[strlen(tmp)+1];
			strcpy(menu_options_text,tmp);
			menu_nitems=4;
		} /* if */ 
		if (menu_current_menu==3) {
			...
			delete menu_options_text;
			sprintf(tmp,"RIGHT : %s\nLEFT : %s\nFIRE : %s\nBACK\n",
					(menu_redefining_key==3 ? "":_strupr(SDL_GetKeyName((SDLKey)right2_key))),
					(menu_redefining_key==4 ? "":_strupr(SDL_GetKeyName((SDLKey)left2_key))),
					(menu_redefining_key==5 ? "":_strupr(SDL_GetKeyName((SDLKey)fire2_key))));
			menu_options_text=new char[strlen(tmp)+1];
			strcpy(menu_options_text,tmp);
			menu_nitems=4;
		} /* if */ 
