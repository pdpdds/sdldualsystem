...
void printobjinfo (int n) {
	...
	wprint (&statvp,38,11,2,_itoa(objs[n].state,tempstr,10));
	wprint (&statvp,2,21,2,   "xd:               ");
	wprint (&statvp,38,21,2,_itoa(objs[n].xd,tempstr,10));
	wprint (&statvp,110,1,2,  "yd:               ");
	wprint (&statvp,140,1,2,_itoa(objs[n].yd,tempstr,10));
	wprint (&statvp,110,11,2, "cnt:              ");
	wprint (&statvp,140,11,2,_itoa(objs[n].counter,tempstr,10));
	if (kindflags[objs[n].objkind]&f_inside)
		wprint (&statvp,110,21,2,"Text Inside");
	wprint (&statvp,248,11,2, "Number: ");
	wprint (&statvp,296,11,2,_itoa(n,tempstr,10));
	};
...
void design (void) {
	...
	do {
		...
		fontcolor (&statvp,3,0);
		wprint (&statvp,296,21,2,_itoa(numobjs,tempstr2,10));
...
//						};
			...
			case 'Y':												// Disalign Y
				...
				_itoa (disy,tempstr,10);
				winput (&statvp,2,11,1,tempstr,16);
				disy=atoi (tempstr);
				_strupr (tempstr); break;
			case 'N':
