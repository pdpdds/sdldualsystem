...
void loadcfg(void) {
	...
	{
		fread(&hiname, 1, sizeof(hiname), cfgfile);
		fread(&hiscore, 1, sizeof(hiscore), cfgfile);
		fread(&savename, 1, sizeof(savename), cfgfile);
		if (fread(&cf, 1, sizeof(cf), cfgfile) == 0) cf.firstthru = 1;
...
void savecfg(void) {
	...
	if (cfgfile) {
		fwrite(&hiname, 1, sizeof(hiname), cfgfile);
		fwrite(&hiscore, 1, sizeof(hiscore), cfgfile);
		fwrite(&savename, 1, sizeof(savename), cfgfile);
		fwrite(&cf, 1, sizeof(cf), cfgfile);
	};
	fclose(cfgfile);
};
...
void loadboard(char* fname) {
	...
	boardfile = fopen(dest, "rb");
	if (!fread(&bd, 1, sizeof(bd), boardfile)) rexit(1);
	if (!fread(&numobjs, 1, sizeof(numobjs), boardfile)) rexit(2);
	for (int i = 0; i < numobjs; i++) {
		if (!fread(&objs[i], 1, 31, boardfile)) rexit(3);
	}
	if (!fread(&pl, 1, sizeof(pl), boardfile)) rexit(4);
	if (!fread(&cnt_fruit, 1, sizeof(cnt_fruit), boardfile)) rexit(5);
	for (c = 0; c < numobjs; c++) {
		if (objs[c].inside_val != 0) {
			fread(&tempint, 1, sizeof(tempint), boardfile);
			objs[c].inside = (char*)malloc(tempint + 1);
			objs[c].inside_val = 1;
			fread(objs[c].inside, 1, tempint + 1, boardfile);
		}
		else {
			objs[c].inside = NULL;
		};
...
void saveboard(char* fname) {
	...
	boardfile = fopen(dest, "wb");				// Was O_BINARY
	if (boardfile < 0) rexit(20);
	if (fwrite(&bd, 1, sizeof(bd), boardfile) == 0) rexit(5);
	fwrite(&numobjs, 1, sizeof(numobjs), boardfile);
	//write (boardfile,&objs,numobjs*sizeof(objs[0]));
	for (int i = 0; i < numobjs; i++) {
		fwrite(&objs[i], 1, 31, boardfile);
	}
	fwrite(&pl, 1, sizeof(pl), boardfile);
	fwrite(&cnt_fruit, 1, sizeof(cnt_fruit), boardfile);
	for (c = 0; c < numobjs; c++) {
		if (objs[c].inside_val != 0) {
			tempint = strlen(objs[c].inside);
			fwrite(&tempint, 1, sizeof(tempint), boardfile);
			fwrite(objs[c].inside, 1, tempint + 1, boardfile);
		};
	};
	fclose(boardfile);
};
...
int loadsavewin(char* msg, char* blankmsg) {
	...
	for (c = 0; c < numsaves; c++)
		wprint(&menu_win.inside, 16, 20 + c * 10, 2, _itoa(c + 1, s, 10));
...
void rest(void) {
	...
	do { checkctrl0(0); } while ((key != 0) || (fire1) || (fire2) || (dx1 != 0) || (dy1 != 0));
	do {
		checkctrl0(0);
...
void dotextmsg(int n, int flg) {
	...
	if (textmsg != NULL) {
		...
		if (linecount <= textys) {
			...
			pageflip(); ourdelay();
			do { checkctrl0(1); } while ((dx1 != 0) || (dy1 != 0) || (key != 0) || (fire1) || (fire2));
...
int askquit(void) {
	...
	do { checkctrl0(0); } while ((key == 0) && (fire1 == 0) && (fire2 == 0) && (dx1 == 0) && (dy1 == 0));
...
void printhi(int newhi) {
	...
	for (c = 0; c < numhighs; c++) {
		wprint(&hiwin.inside, 120 - (strlen(s) * 6), 20 + c * 7, 2,
			_ultoa(hiscore[c], s, 10));
	};

	if (newhi) {
		_ultoa(hiscore[posn], s, 10);
...
int main(int argc, char* argv[])
{
	...
	loadcfg();
	clrscr();
	uncrunch((char*)CFG_WIN, scrnaddr, c_len);
