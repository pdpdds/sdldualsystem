...
/*
 Copie ?l'?ran l'image de la balle num?o i
*/
void showBall(int i)
{
    if (ball[i].utilise==1)
...
/*
  ...
  la balle ?touch?une plateforme sur le cot?ou sur les surfaces horizontales
  sprite1 = sprite de la balle num?o b
...
int ballCollideWithPlatform(int sprite1,int sprite2,int b)
{
	...
	/* si la balle va ?droite*/
	if (ball[b].move>0)	
	{
		if (cote==4) return TOUCH_HORIZONTAL; /* elle touche un cot?droit */		
	...
	/* D'apr? les tests fait plus haut ya forc?ent une collision; mais bon on en renvoie une par d?aut */
...
/* 
   Teste les collisions avec les bords de l'?ran 
*/
...
{
     playSound(4,1);
    /* Cr? une nouvelle balle plus petite qui ira ?gauche */
	int tmp2;
    ...
    /* Cr? une nouvelle balle plus petite qui ira ?droite */	
	tmp2=createBall(ball[a].posx,ball[a].posy,ball[a].type+1,RIGHT,UP);
	ball[tmp2].hauteurmax_cpt = ball[tmp2].hauteurmax-(20);
	ball[tmp2].vel=2.0;
	/* Si la balle <a> qui explose contient un bonus, on le cr? */
	if (ball[a].bonus!=0)
	{
       createBonus(ball[a].bonus,(int)ball[a].posx+10,(int)ball[a].posy+10);
	}
	/* On lib?e la balle (et le sprite associ? qui vient d'exploser */
	ball[a].utilise = 0;
...
*/
	...
	if (ball[i].utilise==1)
	{	
		/* il faut r?up?er la derniere position sans aucune collision 
           Pour cela on teste si on est en collision, si ce n'est pas le
           cas, on mets ?jour last_posx et last_posy. Sinon, on ne mets 
           rien ?jour et on ?les bonnes derni?es coordonn?s de la position
           sans collision.
        */
		...
		for (p=0;p<MAX_SHOOT;p++)
		{
			if (shoot[p].utilise==1)
			{
				...
				if (collide!=0) 
				{
					...
					/* il fo d?ruire la balle EN DERNIER ! Sinon une autre sera reprise ?? position */
					ball[i].utilise = 0;
					if (ball[i].bonus!=0)
					{
