...
typedef struct sballe{
	double posx;		/* Position x dans l'?ran (?initialiser) */
	double posy;		/* Position y dans l'?ran (?initialiser) */
	double last_posx;   /* Derni?e position x sans collision ?l'?ran */
	double last_posy;	/* Derni?e position y sans collision ?l'?ran */
	int hauteurmax;		/* Hauteur maximum en pixel que peut prendre une boule (?initialiser)*/
	int hauteurmax_cpt;	/* Compteur interne (?initialiser ?0) */
	int speedx;		    /* Vitesse de d?lacement horizontal */
	int speedy;		    /* Vitesse de d?lacement verticale (+ la valeur est grande + c lent) */
	int speed_cpt;		/* Compteur interne (?initialiser ?0) */
	int lr;			    /* Direction horizontale -1 = droite    +1 = gauche */
	int ud;			    /* Direction verticale -1 = haut    +1 = bas */
	int coefdivaccell;	/* Coefficient d'acc??ation (+ la valeur est grande + c lent) */
	int utilise;		/* 0 = libre */
	...
	int spriteno;       /* Cette balle est repr?ent? par le sprite num?ro <spriteno> */
	double vel;
	double vel_cst;
	int move;

	int nbtouch; /* variable qui ?ite les balles d'?res bloqu?s */ 
...
typedef struct stir{
	int posx;		/* Position x dans l'?ran (?initialiser) */
	int posy;		/* Position y dans l'?ran (?initialiser) */
	int xbox;
	int ybox;
	int hbox;		/* Hauteur et largeur de la boite de collision */
	int lbox;
