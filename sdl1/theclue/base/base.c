...
static int tcInit(void)
{
	...
	if (StdBuffer0 && StdBuffer1)
	{
		...
		gfxCopyCollToXMS(128, &StdRP0InXMS);    // Menü nach StdRP0InXMS
	...
	while (dskGets(line, sizeof(line)-1, file)) {
		...
		if (pValue = strchr(line, '=')) {
			*pValue++ = '\0';
			...
			if (!_stricmp(pParam, "screenwidth")) {
				Config.gfxScreenWidth = atoi(pValue);
			} else if (!_stricmp(pParam, "screenheight")) {
				Config.gfxScreenHeight = atoi(pValue);
			} else if (!_stricmp(pParam, "fullscreen")) {
				Config.gfxFullScreen = atoi(pValue) ? 1 : 0;
			} else if (!_stricmp(pParam, "scale2x")) {
				Config.gfxScaleNx = atoi(pValue) ? 1 : 0;
			} else if (!_stricmp(pParam, "soundvolume")) {
				Config.SfxVolume = atoi(pValue);
			} else if (!_stricmp(pParam, "musicvolume")) {
				Config.MusicVolume = atoi(pValue);
			} else if (!_stricmp(pParam, "nofontshadow")) {
				Config.gfxNoFontShadow = atoi(pValue) ? 1 : 0;
			} else if (!_stricmp(pParam, "nointro")) {
				Config.NoIntro = atoi(pValue) ? 1 : 0;
			} else if (!_stricmp(pParam, "usejoystick")) {
				Config.UseJoystick = atoi(pValue);
			} else if (!_stricmp(pParam, "demo") && (atoi(pValue) == 42)) {
				GamePlayMode |= GP_DEMO|GP_STORY_OFF;
			} else if (!_stricmp(pParam, "storyoff") && (atoi(pValue) == 42)) {
				GamePlayMode |= GP_STORY_OFF;
			} else if (!_stricmp(pParam, "fullenv") && (atoi(pValue) == 42)) {
				GamePlayMode |= GP_FULL_ENV;
			} else if (!_stricmp(pParam, "leveldesign") && (atoi(pValue) == 42)) {
				GamePlayMode |= GP_LEVEL_DESIGN;
			} else if (!_stricmp(pParam, "guarddesign") && (atoi(pValue) == 42)) {
				GamePlayMode |= GP_GUARD_DESIGN;
			} else if (!_stricmp(pParam, "nosamples") && (atoi(pValue) == 42)) {
				GamePlayMode |= GP_NO_SAMPLES;
			} else if (!_stricmp(pParam, "nocollision") && (atoi(pValue) == 42)) {
				GamePlayMode |= GP_COLLISION_CHECKING_OFF;
			} else if (!_stricmp(pParam, "showrooms") && (atoi(pValue) == 42)) {
				GamePlayMode |= GP_SHOW_ROOMS;
			} else if (!_stricmp(pParam, "moremoney") && (atoi(pValue) == 42)) {
...
int main(int argc, char **argv)
{
	...
	// und den Pfad f?r BuildPathName setzen!
