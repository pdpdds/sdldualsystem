...
int map_x=0,map_y=0;	/* En que habitaci? dentro del mundo		*/ 
int pers_x=0,pers_y=0;	/* Posici? del personaje en la habitaci?	*/ 
int pers_pos=0;
...
bool previous_x_collision=false;	/* Hubo una colisi? en X en el frame anterior? */ 
bool previous_y_collision=false;	/* Hubo una colisi? en Y en el frame anterior? */ 
int hit_time=0;	/* Contador para saber cuando pueden da?rnos de nuevo	*/ 
int character=0;	/* Personaje seleccionado	*/ 
bool live_character[2]={true,true};	/* Si los personajes est? vivos o muertos */ 
...
int n_fired_mines=0;	/* N?de flechas activas	*/ 
...
bool room_demonroom=false,old_room_demonroom=false;	/* Indica si es la habitaci? de un monstruo. */ 
...
//				set_palete((BYTE *)tiles_bmp->get_palete());
	...
	case 6:	if (!old_keyboard[ITEM_KEY] && !old_keyboard[SDLK_RETURN]) {
			...
			break;
	case 7:	/* DENTRO DE ALG? PASAJE, TIENDA, ETC.: */ 
			if (SUBSTATE==0) {
				SUBSTATE=1;
				currently_selecting=0;
				passage_state=0;
			...
			if (!old_keyboard[DOWN_KEY] && keyboard[DOWN_KEY]) {
				STATE=4;
				SUBSTATE=1;
				/* Restaurar la m?ica de juego normal: */ 
				char tmp[80],tmp2[80];
				Sound_release_music();
	...
	case 11:/* MENSAJE DE PERSONAJE MUERTO: */ 
			{
				...
				if (live_character[1] ||
					...
					if (SUBSTATE>=120) { 
						...
						/* Calcular la nueva posici? del personaje en el mapa: */ 
