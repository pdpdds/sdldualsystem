...
/*** Ajoute une entr? dans le tableau des boutons ***/
/*****************************************************/
...
void ChangeVideo(void)
{
	...
	if (sdlVideoInfo->vfmt->BitsPerPixel == 8) {
		cerr << "Impossible d'utiliser 8bits pour la vid? !" << endl;
		exit(-1);
	}
...
#ifndef LINUX
	int vOption;
	if (Pref.FullScreen)   vOption = SDL_SWSURFACE; // Bug acc?aux bits pour les cordes
	...
	if (sdlVideo == NULL) {
		cerr << "Impossible de passer dans le mode vid? 800x600 !" << endl;
		exit(-1);
	}
			...
			case SDL_KEYDOWN:
				if (event.key.state == SDL_PRESSED) {
					...
					case SDLK_KP_ENTER:
						...
						break;
					default:
						key = event.key.keysym.unicode & 0x7F; // Prend le caracataire correspondant ?la touche
						if (CharExiste(key) == true) { // Si la caractaire existe bien
							for (i = 2; i >= 0; i--) MCode[i + 1] = MCode[i]; // d?ale le code
							MCode[0] = key;
...
eMenu Menu::SDLMain_Options(void)
{
	...
	InitMain_Options(); // Pr?are le menu
...
/*** SDL Main Menu Choix de la difficult?***/
/********************************************/
...
eMenu Menu::SDLMain_Niveau(void)
{
	...
	if (Niv == 0) return mMenuSpeed; // Inutile si la premi?e fois
...
eMenu Menu::SDLMain_Score(bool EditScore)
{
	...
	// Cherche le num?o du score ?remplacer si edition des scores
	if (EditScore) {
		for (i = 7; i >= 0; i--) if (Pref.Sco[i].Score < Pref.Score) NEdit = i;
		if (NEdit == -1) return mMenu;

		if (NEdit < 7) // Si doit fair un d?alage
			for (i = 7; i > NEdit; i--) {
	...
	do {
			...
			case SDL_KEYDOWN: // Prend un touche au clavier
				if (event.key.state == SDL_PRESSED) {
					...
					case SDLK_F12: // Sauve un screenshot
						SDL_SaveBMP(sdlVideo, "screenshot.bmp");
						break;
					case SDLK_ESCAPE: // Valide l'entr?
					case SDLK_RETURN:
					case SDLK_KP_ENTER:
						return mMenu;
		...
		if (EditScore) { // G?e l'affiche pour l'?ition des scores
...
/*** Centre les fl?hes sur le boutton ***/
/*****************************************/
