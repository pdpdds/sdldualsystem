...
{
  // Initialise les coordonn?s de la sourie
  Px=400;
  Py=300;
  SDL_WarpMouse(Px,Py);
}
...
void Mouse::GetEvent(SDL_Event &event,int &pPy)
{
  ...
  case SDL_MOUSEMOTION: // Si mouvement de la sourie
    ...
    if(tPy) { // Si bien une table
      i=0;
      while(tPy[i].DepX!=-1) { // Fait toutes les coordonn?s
  ...
  case SDL_MOUSEBUTTONDOWN:
	...
	i=0;
	while(Bo[i].DepX!=-1) { // Fait toutes les coordonn?s
...
void Mouse::Affiche(int NumVideo)
{
  ...
  // Corrige la position du curseur au cas ou d?orde de l'?ran
  if(X<5) X=5;
