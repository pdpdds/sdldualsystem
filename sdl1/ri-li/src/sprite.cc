...
char Langue[31][16]; // M?orise les noms des langues
int NTextes = 0;
...
{
	static int NumAf = -1; // Num?o su sprite affich?  int i,Old;
	int Hor;

	if (AfficheC == true) {
		Hor = SDL_GetTicks();
		int i = (Hor / (1000 / 25)) % Sprites[chargeur].N; // Calcule le num?o su sprite ?afficher
...
bool LoadLangue()
{
	...
	delete[] Buf; // Lib?e la m?oire du fichier des sprites
...
bool Sprite::Load(unsigned char* Buf, long& P)
{
	...
	for (i = 0; i < N; i++) {
		...
		if (Image[i] <= NULL) {
			cerr << "Impossible de cr?r une Surface SDL!" << endl;
			return false;
		}
