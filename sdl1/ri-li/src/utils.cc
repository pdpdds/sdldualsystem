...
long ChargeFichier(const char *Path,unsigned char *&Buf)
{
  ...
  long L=ftell(file); // r?up?e la longueur
  fseek(file,0,0);
...
bool SauveFichier(const char *Path,char *Buf,long L)
{
  ...
  file=_lcreat(Path,0);
  if(!file) {
    cerr <<"ERREUR: Impossible de cr?r le fichier '"<<Path<<"'"<<endl;
    return false;
  }
  ...
  if(Lec!=L) {
    cerr <<"Probl?e d'ecriture du fichier '"<<Path<<"' ecris="<<Lec<<" au lieux de ="<<L<<endl;
    return false;
  }
