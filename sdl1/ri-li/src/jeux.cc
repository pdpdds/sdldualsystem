...
int MasqueK; // Masque pour les touches de d?lacement
...
eMenu Jeux::SDLMain(void)
{
	...
	// Met le options de d?art du joueur
	...
	do {
		...
		// G?e l'appui des touches
		if (Key && Lo.Mort == -1) PrendTouche(Key);

		// G?e les Horloges et la pose
...
bool Jeux::Load(int NivN)
{
	...
	// Met la vitesse suivant difficult?  
	switch (Pref.Difficulte) {
	case Easy:
...
/*** M?orise une touche dans le buffet des touches ***/
/******************************************************/
void Jeux::BufTouche(int Tou)
{
	...
	if (n) { // Si changement doit faire un d?alage
		while (n) {
			Touche[n] = Touche[n - 1];
			n--;
		}
	...
	if (n < 3) { // Si changement doit faire un d?alage
		while (n < 3) {
			Touche[n] = Touche[n + 1];
			n++;
		}
		Touche[3] = Tou; // M?orise la touche
	}
}
...
void Jeux::AfficheEcran(void)
{
	...
	// Pr?are pour nouvelle Affichage
