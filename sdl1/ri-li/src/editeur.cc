	...
	do {
		...
		while (SDL_PollEvent(&event)) {
			...
			case SDL_MOUSEBUTTONUP:
				Boutton = false;
				if (TypeB != -1 && Option == deco && cx >= LT) { // Si doit effacer une d?oration
					Niveau.T[NumN].NDeco--;
				}
				TypeB = -1;
				break;
		...
		// G?e l'appuis du boutton de la sourie
		cx = Sourie.Px / D_Case;
		cy = Sourie.Py / D_Case;
		...
		if (Boutton && cx < LT)
			switch (Option) {
			case deco:
				if (TypeB == -1) { // Si premi?e fois que appuis sur la touche
					for (i = 0; i < Niveau.T[NumN].NDeco; i++) { // Recherche si d?oration proche du clic
					...
					if (TypeB == -1) { // Si doit fair un nouveau d?or
						Niveau.T[NumN].NDeco++;
				...
				else { // Si pas la premi?e fois remplace
					Niveau.T[NumN].Deco[(Niveau.T[NumN].NDeco - 1)].NumSpr = NumDeco;
					Niveau.T[NumN].Deco[(Niveau.T[NumN].NDeco - 1)].x = Sourie.Px;
					Niveau.T[NumN].Deco[(Niveau.T[NumN].NDeco - 1)].y = Sourie.Py;
				}
...
void Editeur::Affiche()
{
	...
	// Affiche le d?art de la locomotive
