...
struct s_Dim {
  int L,H;
  int cx,cy; // Centre de l'image apr? recadrage
...
/*** Fonctions ***/
/*****************/
void AfficheChargeur(void);   // Affiche le chargeur sur la page de d?art
...
class Sprite
{
 public:
  ...
  void EffaceCarre(int dx,int dy,int fx,int fy,SDL_Surface *Fond); // Efface un carr??l'ecran
  bool Nouveau(int Lx,int Ly); // Alloue un nouveau sprite vide sans transparence
  void Delete(void); // Efface la m?oire du sprite
