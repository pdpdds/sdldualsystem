...
/*** Structure des preferences G??ales ***/
struct sPreference {
  e_Difficulte Difficulte; // Difficult?de la partie (Vitesse)
  ...
  int NiveauMax;
  int FullScreen;
  int Langue;        // Langue ?afficher
  int NLangues;      // Nombre de langues disponible
  float Volume;        // Volumes audio
  float VolumeM;       // Volume de la music
  struct sScore Sco[8]; // M?orise les scores
};

/*** Valeur retourn?s pour le menu princiaple ***/
enum eMenu {
  mMenu,
  mMenuJeux,
  mMenuSpeed,
